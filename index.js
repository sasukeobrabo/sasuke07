...
const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            ...
            + 'TEL;type=CELL;type=VOICE;waid=553299664033 :+55 32 9966-4033\n' // ID do WhatsApp + número de telefone
            + 'END:VCARD'
/******END OF VCARD INPUT******/
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Oiin sou o bot do sasuke@${num.split('@')[0]}\nBem vindo ao grupo seja ativo🙂❤️*${mdata.subject}*`
				...
				teks = `Xauzinho foi de ralo hahaha @${num.split('@')[0]}👋😂`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '🐧 Calmae poha to fazendo lek 🐧',
				success: '✔️ Sucesso ✔️',
                                levelon: '❬ ✔ ❭ *habilitar Level*',
				leveloff: ' ❬ X ❭  *desabilitar Level*',
				levelnoton: '❬ X ❭ *level não ativo*',
				levelnol: '*Pqp kskst level* 0 O cara e ghost hahaha',
				error: {
			...
			const ownerNumber = ["553299664033@s.whatsapp.net"] // replace this with your number
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        const NomerOwner = '553299664033@s.whatsapp.net'
                        /******ApiKey Input******/
	...
	case 'kbbi':
		...
		if (args.length < 1) return reply ('*PARCEIROS*: \nEaxy: https://youtube.com/c/EAXYFF\nYamii: https://youtube.com/c/TioYamiiroivemsmpaki\nSaad: https://youtube.com/channel/UCtX3fzhC-teLUpaS7Xz8nKA\nKazuma: https://youtube.com/channel/UChRgBlar0IKGAsseuBeNRgA')
				...
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘 S∆SUKE BOT 〙', members_id, true)
					break
                case 'tagall2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 Mencionando Todos 〙✪══\n╠➥'+teks+'╚═〘 S∆SUKE BOT 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
